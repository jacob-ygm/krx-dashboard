...
if os.path.exists("model_performance.csv"):
    ...
    m1, m2 = st.columns(2)
    m1.metric("모델 예측 정확도", f"{acc_v*100:.1f}%")
    m2.metric("상승 적중 정밀도(Precision)", f"{prec_v*100:.1f}%")
...
if os.path.exists("full_result.csv"):
    ...
    st.divider()
    st.subheader("🧐 종목별 추천 근거 분석")
    sel = st.selectbox("분석 종목 선택", top10['name'].tolist())
    ...
    with c1:
        ...
        st.info(f"모델 정밀도({prec_v*100:.1f}%) 기반 검증 완료.")
    with c2:

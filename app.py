...
st.set_page_config(page_title="2026 AI 인프라 분석", layout="wide")
...
if os.path.exists("full_result.csv") and os.path.exists("news_info.csv"):
    ...
    st.subheader("🏁 종합 예측 순위 (Macro + Chart + Sentiment)")
    ...
    with col1:
        st.write(f"### 📊 {sel} 지표 분석")
        row = df[df['name'] == sel].iloc[0]
        st.metric("상승 확률", f"{row['prob']*100:.1f}%")

...
def main():
    ...
    with tab_guide:
        ...
        g3.metric("검증 승률", "67.1%", help="Digital Twin 216건 / 20일 보유 기준. 실전 기대 60~64%")
...
| 순위 | 조건 | 행동 |
|---|---|---|
| ① | 손절가 터치 | **즉시 전량 매도. 예외 없음** |
| ② | 목표가 도달 | 절반 매도 + 남은 절반 손절가를 매수가로 상향 |
| ③ | 매수 후 20거래일 경과 | 손익 무관 **전량 매도** |

세 개 다 아니오 → **아무것도 하지 않음** (차트 보지 마세요)

> **왜 20거래일인가**
> 검증된 67% 승률은 "20거래일 후 종가 매도" 기준입니다.
> "조금 더 오를 것 같아서" 연장하는 순간 통계 밖입니다.
            """)
